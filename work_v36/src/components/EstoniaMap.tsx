...
function spruceStyle(
  ...
  const { weather } = center ? weatherForLocation(center.lat, center.lng, weatherStats) : { weather: null };
  const insideDangerZone = featureCenterInsideDanger(feature, dangerPolygons);
  if (!insideDangerZone || !center) return forestOutsideSpreadStyle();
...
function createExactUnionDangerZoneLayer(L: any, spreadUnionGeoJson: any) {
...
function ndviRiskValue(ndvi: number | null) {
  ...
  if ((ndvi as number) <= 0.3) return { value: 3, label: "NDVI 0–0,3" };
  if ((ndvi as number) <= 0.7) return { value: 2, label: "NDVI 0,3–0,7" };
  return { value: 1, label: "NDVI 0,7–1" };
}
...
function assessForestInfectionRisk(options: {
  ...
  if (options.insideDangerZone === true) {
    factors.push("mets lõikub arvutatud levialaga; levikurisk on otseselt aktiivne");
  } else if (options.insideDangerZone === false) {
    factors.push("mets jääb arvutatud levialast välja; risk tuleb püsivatest tunnustest ja kaugusest");
  } else {
  ...
  if (ndviRisk) {
    ...
    factors.push("NDVI puudub nii metsa atribuutides kui plot_medians.json failis; NDVI-d riskiskooris ei arvestatud");
  }
...
function forestOutsideSpreadStyle() {
  return {
    pane: "sprucePane",
    color: "#14532d",
    weight: 0.55,
...
function forestRiskColor(score: number | null) {
  const value = clamp(Number(score ?? 0), 0, 100);

  // Levikuala sees olev terve kuusemets ei lähe enam punaseks.
  // Risk kuvatakse kollasest tumedama oranžini: väiksem risk = hele kollane, suurem risk = tugevam oranž.
  const hue = Math.round(55 - (25 * value) / 100); // 55 = kollane, 30 = oranž
  const lightness = Math.round(76 - (30 * value) / 100);
  const fillColor = `hsl(${hue}, 95%, ${lightness}%)`;
  const strokeColor = value >= 70 ? "#9a3412" : value >= 45 ? "#ca8a04" : "#eab308";

  return {
    color: strokeColor,
    fillColor,
    fillOpacity: value >= 70 ? 0.86 : value >= 45 ? 0.78 : 0.68,
    weight: value >= 70 ? 0.85 : 0.65,
    opacity: 0.95,
  };
}

export function EstoniaMap({ area, showWeather, showDamage, showDangerZones, showSpruceForests, onMapClick, onWeatherSummary }: EstoniaMapProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const markerIcon = (synced: boolean) =>
        L.divIcon({
          className: "",
          html: `<div style="width:16px;height:16px;border-radius:9999px;border:2px solid #fff;box-shadow:0 0 0 2px rgba(15,23,42,0.22);background:${synced ? '#16a34a' : '#ef4444'}"></div>`,
          iconSize: [16, 16],
          iconAnchor: [8, 8],
        });
      ...
      const syncBtn = makeButton("Sünkrooni märgid");
      const statusLine = document.createElement("div");
      statusLine.style.fontSize = "12px";
      statusLine.style.lineHeight = "1.4";
      statusLine.style.color = "#cbd5e1";
      statusLine.textContent = "Pikk vajutus lisab täpi ja salvestab selle lokaalselt.";
      ...
      const renderOfflineMarkers = () => {
        ...
        for (const marker of offlineMarkersRef.current) {
          const leafletMarker = L.marker([marker.lat, marker.lng], { icon: markerIcon(marker.synced) });
          leafletMarker.bindPopup(
            `<strong>${marker.synced ? "Sünkroonitud täpp" : "Ootel täpp"}</strong><br>Koordinaadid: ${marker.lat.toFixed(5)}, ${marker.lng.toFixed(5)}<br>Lisatud: ${new Date(marker.createdAt).toLocaleString("et-EE")}`,
          );
      ...
      const addOfflineMarker = (lat: number, lng: number, synced = false) => {
        const marker: OfflineMarkerRecord = {
          id: `${offlineClientIdRef.current}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
          lat,
          lng,
          createdAt: new Date().toISOString(),
          clientId: offlineClientIdRef.current,
          synced,
          syncedAt: synced ? new Date().toISOString() : null,
        };
        offlineMarkersRef.current = [...offlineMarkersRef.current, marker];
        persistOfflineState();
        setStatus(synced ? "Täpp lisatud ja sünkroonitud." : "Täpp lisatud lokaalselt. Sünkroniseerin, kui side taastub.");
        if (navigator.onLine) {
      ...
      const mergeServerMarkers = (incoming: OfflineMarkerRecord[]) => {
        const byId = new Map<string, OfflineMarkerRecord>();
        for (const marker of offlineMarkersRef.current) byId.set(marker.id, marker);
        ...
        offlineMarkersRef.current = [...byId.values()].sort((a, b) => a.createdAt.localeCompare(b.createdAt));
        persistOfflineState();
      };

      const syncOfflineMarkers = async () => {
        if (!navigator.onLine) {
          setStatus("Side puudub. Märgid jäävad lokaalselt alles.");
          return;
        }

        const pending = offlineMarkersRef.current.filter((marker) => !marker.synced);
        if (!pending.length) {
          setStatus("Kõik märgid on juba sünkroonitud.");
          return;
        }

        try {
          const response = await fetch(`${BASE_URL}api/offline-marks`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ clientId: offlineClientIdRef.current, markers: pending }),
          });
          if (!response.ok) throw new Error(`HTTP ${response.status}`);
          const payload = (await response.json().catch(() => null)) as { markers?: OfflineMarkerRecord[] } | null;
          const syncedIds = new Set((payload?.markers ?? pending).map((marker) => marker.id));
          offlineMarkersRef.current = offlineMarkersRef.current.map((marker) =>
            syncedIds.has(marker.id)
              ? { ...marker, synced: true, syncedAt: marker.syncedAt ?? new Date().toISOString() }
              : marker,
          );
          if (payload?.markers?.length) mergeServerMarkers(payload.markers);
          persistOfflineState();
          setStatus(`Sünkroonitud ${pending.length} märki.`);
        } catch (error) {
          console.warn("Offline marker sync failed", error);
          setStatus("Sünkroon ebaõnnestus. Märgid jäid lokaalselt alles.");
        }
      };
      ...
      const loadSharedMarkers = async () => {
        ...
        try {
          const response = await fetch(`${BASE_URL}api/offline-marks`, { cache: "no-store" });
          if (!response.ok) return;
          const payload = (await response.json().catch(() => null)) as { markers?: OfflineMarkerRecord[] } | null;
          if (payload?.markers?.length) mergeServerMarkers(payload.markers);
        } catch (error) {
          console.warn("Shared offline markers load failed", error);
        }
      };
      ...
      map.on("contextmenu", (event: any) => {
        ...
        addOfflineMarker(event.latlng.lat, event.latlng.lng);
      });

      map.on("mousedown touchstart", (event: any) => {
        const latlng = event?.latlng;
        if (!latlng) return;
        longPressLatLngRef.current = latlng;
        if (longPressTimerRef.current) clearTimeout(longPressTimerRef.current);
        longPressTimerRef.current = setTimeout(() => {
          if (longPressLatLngRef.current) {
            addOfflineMarker(longPressLatLngRef.current.lat, longPressLatLngRef.current.lng);
          }
        }, 700);
      });

      map.on("mouseup touchend dragstart movestart zoomstart", () => {
        if (longPressTimerRef.current) clearTimeout(longPressTimerRef.current);
      ...
      if (navigator.onLine) {
        ...
        setStatus("Offline. Varem salvestatud märgid on nähtavad ja muudetavad.");
      }

      const onNetworkChange = () => {
        if (navigator.onLine) {
          setStatus("Side taastunud. Sünkroniseerin märgid.");
          void syncOfflineMarkers();
          void loadSharedMarkers();
        } else {
      ...
      cleanupOfflineUi = () => {
        ...
        map.off("mouseup touchend dragstart movestart zoomstart");
      ...
      try {
        ndviLookupRef.current = await fetch(dataUrl("plot_medians.json")).then((r) => r.json()) as NdviLookup;
      } catch (error) {
        ndviLookupRef.current = {};
        console.warn("NDVI faili laadimine ebaõnnestus, risk arvutatakse ilma NDVI-ta", error);
      }
      try {
      ...
      dangerPolygonsRef.current = dangerPolygons;
      damageCentersRef.current = damageCenters;
      if (dangerPolygons.length) {
        layersRef.current.dangerZones.addLayer(createExactUnionDangerZoneLayer(L, spreadUnion));
      }
        ...
        const chunkLayer = L.geoJSON({ type: "FeatureCollection", features: uniqueFeatures }, {
          ...
          onEachFeature: (feature: any, layer: any) => {
            ...
            layer.on("click", (event: any) => {
              ...
              const center = layer.getBounds ? layer.getBounds().getCenter() : event.latlng;
              const { county, weather } = weatherForLocation(center.lat, center.lng, weatherStatsRef.current);
              const insideDangerZone = featureCenterInsideDanger(feature, dangerPolygonsRef.current);
              const risk = assessForestInfectionRisk({
                properties: p,
                lat: center.lat,
                lng: center.lng,
