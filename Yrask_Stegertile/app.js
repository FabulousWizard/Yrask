...
function mostCommonWindDirection(rows, bucketSize = 10) {
  const buckets = new Map();

  for (const row of rows) {
    if (!Number.isFinite(row.windDirection)) continue;

    const rounded = Math.round(row.windDirection / bucketSize) * bucketSize;
    const direction = ((rounded % 360) + 360) % 360;

    const current = buckets.get(direction) || { count: 0, speedSum: 0 };
    current.count += 1;
    current.speedSum += Number.isFinite(row.windSpeed) ? row.windSpeed : 0;
    buckets.set(direction, current);
  }
  ...
  for (const [direction, data] of buckets.entries()) {
    if (data.count > bestCount || (data.count === bestCount && data.speedSum > bestSpeedSum)) {
      bestDirection = direction;
      bestCount = data.count;
      bestSpeedSum = data.speedSum;
    }
  }
...
function aggregateRecordsToCountyStats(records) {
  ...
  return [...buckets.values()].map(({ county, rows }) => {
    ...
    return {
      ...
      windDirection: mostCommonWindDirection(windRows),
      stations: rows
    };
...
async function loadRealtimePayload() {
  ...
  const records = parsed.observations
    .map(normalizeRealtimeObservation)
...
function enumerateHours() {
  return Array.from({ length: 24 }, (_, i) => String(i).padStart(2, "0"));
}

async function loadDayPayload(date = new Date()) {
  const dateKey = dateKeyTallinn(date);
  const cacheKey = `day-${dateKey}`;
  if (payloadCache.has(cacheKey)) return payloadCache.get(cacheKey);

  setDebug(`Laen terve päeva tuuleandmeid (${dateKey})...`);

  const dailyRaw = await fetchJsonWithTimeout(
    `${API_BASE}/v1/combinedWeatherData/observationDataDaily?date=${encodeURIComponent(dateKey)}&hour=12`
  );

  const dailyRows = extractEntries(dailyRaw).map(normalizeDailyObservation);

  const hours = enumerateHours();
  const windBundles = await mapWithConcurrency(hours, 6, async (hour) => {
    try {
      const [windRaw, windMapRaw] = await Promise.all([
        fetchJsonWithTimeout(
          `${API_BASE}/v1/wind/observationWind?date=${encodeURIComponent(dateKey)}&hour=${encodeURIComponent(hour)}`
        ),
        fetchJsonWithTimeout(
          `${API_BASE}/v1/wind/observationWindMap?date=${encodeURIComponent(dateKey)}&hour=${encodeURIComponent(hour)}`
        )
      ]);

      const windRows = extractEntries(windRaw).map(normalizeWindObservation);
      const windMapRows = extractEntries(windMapRaw).map(normalizeWindMapObservation);
      return combineDailyAndWindRows([], windRows, windMapRows);
    } catch (error) {
      console.warn(`Tuuleandmed ebaõnnestusid ${dateKey} ${hour}:`, error);
      return [];
    }
  });

  const allWindRows = windBundles.flat();
  const bundle = combineDailyAndWindRows(dailyRows, allWindRows, []);
